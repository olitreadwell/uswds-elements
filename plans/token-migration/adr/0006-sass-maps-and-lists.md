...
| `$system-properties` (subset: `box-shadow`, `opacity`, `z-index`, `order`, `flex`, `flex-direction`, `flex-wrap`, `gap`, `letter-spacing`, `line-height`) | `_properties.scss`                     | `tokens/system/{z-index,opacity,shadow,flex,gap}/*.json`, extends `tokens/typography/*` (letter-spacing, line-height) — see ADR-0009 for the scope boundary within this map              |
